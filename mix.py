...
import os
#os.environ["esmfmkfile"] = r"C:/Users/17865/anaconda3/envs/esmpy/Library/lib/esmf.mk"
#import ESMF
#import xesmf as xe
import pickle
import sys
...
# 其他指定路径
# 其他指定路径
CB05_DIR = "C:/Users/17865/Desktop/MIX/MIX2010/CB05/Output/"    #meic cb05文件存储路径。内部为YYYYMM的文件夹，文件夹内存放每个月都meic排放源文件
wrfinput_file = "C:/Users/17865/Desktop/MIX/INPUT/wrfinput_d02"   #wrfinput文件位置
wrfchemi_save_dir = "C:/Users/17865/Desktop/MIX/OUTPUT/"   #wrfchemi文件存放目录

# meic经纬度,格点的位于每个网格的中心点
lon = np.arange(40.125, 179.875, 0.25, dtype=np.float32)
lat = np.arange(-20.125,  89.875, 0.25, dtype=np.float32)
        ...
        for j in range(jsize):
            obj = {'type': 'Polygon',
                   'coordinates': [  # 网格的四个顶点经纬度
                       [[startlon, lat[i, j] - res / 2.0], [startlon, lat[i, j] + res / 2.0],
                        [startlon + res, lat[i, j] + res / 2.0], [startlon + res, lat[i, j] - res / 2.0]]
                   ]
                   }
...
def meic2wrf(lon_inp, lat_inp, lon, lat, emis, ):  # lon/lat_inp: model; lon/lat: meic; emis: meic emis
    ...
    def inp(ix, iy, dx, dy, cdx,
            cdy):  # put small function under meic2wrf function, or variables in small functions are global.
        # area_ratio
        return emis[ix, iy] * cdx * cdy + emis[ix, iy + 1] * cdx * dy + emis[ix + 1, iy + 1] * dx * dy + emis[
            ix + 1, iy] * dx * cdy
...
def avg_hour(iemiss,emiss_year,emiss_month):
    ...
    '''
    avg_hour_count = 0
    start_time = datetime(int(emiss_year),int(emiss_month),10,0)   #本月的开始时间,bjt
...
def convert_unit(var,value,iemiss,emiss_year="2010",emiss_month="6"):
    '''
        var:  要变化的变量名
        value: 值，二维数组
        iemiss: 排放源种类(5种）
    ...
    elif var in ['BC', 'OC', 'PM2.5', 'PM10', ]:  # aerosol: ton/(grid.month) to ug/(m2.s)
...
def make_wrfchemi(x):
    ...
    logger.info("start generating " + wrf_time_utc.strftime("%Y-%m-%d_%H_00_00.nc"))

    ncfile = savedir + "/" + wrf_time_utc.strftime(
        "%Y/%m/%d") + "/wrfchemi_d" + domain_id + "_" + wrf_time_utc.strftime("%Y-%m-%d_%H_00_00.nc") + ""
    if os.path.exists(ncfile) and os.path.getsize(ncfile) > 56700000:
        ...
        for spec in interp_meic_emission.keys():
            # sum_list = {}
            for iemiss in ["industry", "power", "residential", "transportation","agriculture"]:
                try:
                    # 各排放源直接相加作为总排放,
                    # 系数乘在这里: 日变化，星期变化，高度层变化
                    ## 周
                    interp_meic_emission[spec][iemiss]["base"] *= week_emiss_factor[iemiss][
                        int(wrf_time_bjt.strftime("%w"))]
                    ## 日变化
                    interp_meic_emission[spec][iemiss]["base"] *= emission_time_distribution[iemiss][
                        int(wrf_time_bjt.strftime("%H"))]
                    ##高度层变化
                    interp_meic_emission[spec][iemiss]["levels"] = np.zeros(
                        (len(emission_height_distribution["power"]), lon_inp.shape[0], lon_inp.shape[1]))
                    for ilevel in range(len(emission_height_distribution["power"])):
                        interp_meic_emission[spec][iemiss]["levels"][ilevel, ...] = interp_meic_emission[spec][iemiss][
                                                                                        "base"] * \
                                                                                    emission_height_distribution[
                                                                                        iemiss][ilevel]
            ...
            for ilevel in range(len(emission_height_distribution["power"])):
                ...
                interp_meic_emission[spec]["all"]["levels"] = np.zeros(
                    (len(emission_height_distribution["power"]), lon_inp.shape[0], lon_inp.shape[1]))
                for iemiss in ["agriculture", "industry", "power", "residential", "transportation"]:
                    try:
        ...
        wrfchem_emission = {}
        print(interp_meic_emission.keys())
        ...
        wrfchem_emission["E_ALD"] = interp_meic_emission['CB05_ALD2']["all"]["levels"] + interp_meic_emission['CB05_ALDX']["all"][
            "levels"]  # ald
        ...
        wrfchem_emission["E_CSL"] = interp_meic_emission['CB05_ISOP']["all"]["levels"] * 0.4 + \
                                    interp_meic_emission['CB05_OLE']["all"]["levels"] * 0.3  # csl
        ...
        wrfchem_emission["E_PM_10"] = interp_meic_emission['PM25']["all"]["levels"] + \
                                      interp_meic_emission['PM10']["all"]["levels"]  # pm10
        ...
        for ivar in wrfchem_emission.keys():
            if ivar in ["E_PM25I", "E_PM25J", "E_SO4I", "E_SO4J", "E_NO3I", "E_NO3J", "E_ORGI", "E_ORGJ", "E_ECI",
                        "E_ECJ", "E_PM_10"]:
        ...
        ds.to_netcdf(
            savedir + "/" + wrf_time_utc.strftime("%Y/%m/%d") + "/wrfchemi_d" + domain_id + "_" + wrf_time_utc.strftime(
                "%Y-%m-%d_%H_00_00.nc") + "")
        # del ds_dict
        # del ds
        # gc.collect()
        logger.success("finish generating " + wrf_time_utc.strftime("%Y-%m-%d_%H_00_00.nc"))
    return
...
def parallel_make_wrfchemi(start_time, end_time, n_jobs=-1):
    ...
    for itime in timelist:
        ...
        if not itime_bjt.strftime("%m") in month_already:
            make_interp_meic_emission("2010", itime_bjt.strftime("%m"), md5value, lon_inp, lat_inp)
...
if __name__ == '__main__':
    start_time = datetime(2018, 6, 10, 0)
    end_time = datetime(2018, 6, 20, 0)
